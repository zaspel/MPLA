...
int main(int argc, char* argv[])
{
	...
	curandGenerateUniformDouble(gen, x.data, x.cur_proc_row_count);
        mpla_generic_dgemv(&b, &A, &x, mpla_dgemv_core_hmatrix, &instance);
...
//	cudaEventRecord(start, 0);
	...
	if (instance.is_parent)
		TIME_test_start;
	mpla_generic_BiCGSTAB_with_checkpoint_restart(&b, &A, &x, max_iter, tol, mpla_dgemv_core_hmatrix, iterations_until_checkpoint, restart, &instance);
	MPI_Barrier(instance.comm);
	if (instance.is_parent)
